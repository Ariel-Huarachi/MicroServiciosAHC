...
export const facturasByCliente = async (req, res) => {
  const clienteId = parseInt(req.params.clienteId, 10);
  const cliente = await repo().findOneBy({ id: clienteId });
  if (!cliente) return res.status(404).json({ message: 'Cliente no encontrado' });
  const facturas = await facturaRepo().find({ where: { cliente: { id: clienteId } } });
  res.json(facturas);
};
